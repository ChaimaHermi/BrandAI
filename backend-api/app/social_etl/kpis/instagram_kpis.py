...
async def _fetch_active_connection_ids(conn: asyncpg.Connection) -> list[int]:
    ...
    if has_is_active:
        rows = await conn.fetch(
            ...
            WHERE platform IN ('instagram_business', 'instagram')
...
async def compute_instagram_kpis(pool: asyncpg.Pool) -> int:
    aggregate_query = f"""
{SYNCED_PERIOD_CTE}
        posts_with_reach AS (
            SELECT sp.*
    ...
    async with pool.acquire() as conn:
        ...
        for connection_id in connection_ids:
            await _compute_instagram_kpis_for_connection(conn, connection_id, aggregate_query, upsert_query)
            updated += 1
        return updated


async def compute_instagram_kpis_for_connection(pool: asyncpg.Pool, connection_id: int) -> None:
    aggregate_query = f"""
{SYNCED_PERIOD_CTE}
        posts_with_reach AS (
            SELECT sp.*
    ...
    upsert_query = f"""
        ...
        ) VALUES ($1, '{PERIOD_TYPE}', $2, $3, $4, $5, $6, $7, $8, NULL, $9, NULL, $10)
        ON CONFLICT (connection_id, period_type, period_start) DO UPDATE SET
            period_end = EXCLUDED.period_end,
            followers_count = EXCLUDED.followers_count,
            posts_count = EXCLUDED.posts_count,
...
async def _compute_instagram_kpis_for_connection(
    conn: asyncpg.Connection,
    connection_id: int,
    aggregate_query: str,
    upsert_query: str,
    ...
    logger.info("instagram_kpis upserted connection_id=%s", connection_id)
