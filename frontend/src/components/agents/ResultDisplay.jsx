...
function StatCard({ label, value, accentColor = "text-[#7C3AED]" }) {
  return (
    <div className="rounded-[12px] border border-[#E5E7EB] border-l-[3px] border-l-[#7C3AED] bg-[#F9FAFB] p-4">
      <p className="text-sm text-[#6B7280]">{label}</p>
      <p className={`mt-1 text-xl font-semibold ${accentColor}`}>{value}</p>
    </div>
  );
}
  ...
  if (status === "waiting")
    return (
      <Card className="flex flex-col items-center justify-center py-16" hover={false}>
        <p className="text-[#6B7280]">En attente de génération...</p>
      </Card>
    );
  ...
  if (status === "running" && data?.typewriterLines)
    return (
      <Card hover={false} className="border border-[#E5E7EB] bg-white">
        ...
        <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-[#F9FAFB]">
