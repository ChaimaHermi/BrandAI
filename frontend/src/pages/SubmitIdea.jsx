          ...
          <div className="w-full max-w-[700px] bg-white rounded-xl border border-[#E5E7EB] shadow-sm p-6 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-[#6B7280]">Donnez un nom court et mémorable</p>
                {name.trim().length > 0 && name.trim().length < 2 && (
                  <p className="mt-0.5 text-xs text-red-500">Minimum 2 caractères</p>
                )}
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label htmlFor="idea-audience" className="mb-1.5 block text-sm font-medium text-[#111827]">Public cible</label>
                  <input
                    id="idea-audience"
                    type="text"
              ...
              {/* Description + counter */}
              <div>
                <label htmlFor="idea-desc" className="mb-1.5 block text-sm font-medium text-[#111827]">Description de votre idée</label>
                <textarea
                  id="idea-desc"
                  value={description}
                ...
                <div className="mt-1 flex items-center justify-between gap-2">
                  ...
                  <span className={`text-xs tabular-nums ${descNearLimit ? "text-amber-600" : "text-[#6B7280]"}`}>
                    {descCount} / {MAX_DESC_LENGTH}
                  </span>
                </div>
                {description.trim().length > 0 && description.trim().length < 20 && (
                  <p className="mt-0.5 text-xs text-red-500">Minimum 20 caractères</p>
                )}
              </div>
